...
type Game struct {
	// unexported fields - so they are unchangeable in middle of game
	nDecks          int
	nHands          int
	blackjackPayout float64
...
func MoveSplit(g *Game) error {
	cards := g.currentHand()
	if len(*cards) != 2 {
		return errors.New("You can only split if your hand consists of two cards")
	}
	if (*cards)[0].Rank != (*cards)[1].Rank {
		return errors.New("In order to split, both cards must have the same value")
	}
...
func MoveDouble(g *Game) error {
	if len(*g.currentHand()) != 2 {
		return errors.New("Can not double down with more than 2 cards.")
	}
	g.playerBet *= 2
	MoveHit(g)
	return MoveStand(g)
...
func endRound(g *Game, ai AI) {
	...
	for hi, hand := range g.player {
		...
		case pScore > dScore:
		case dScore > pScore:
			// Logic is same as what winnings equals in "You busted"
			winnings *= -1
		case pScore == dScore:
			winnings = 0
		}
